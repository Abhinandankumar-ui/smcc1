    ...
    <div className='max-w-7xl mx-auto px-6 py-14'>
      ...
      <div className='text-center max-w-3xl mx-auto mb-10'>
        ...
        <p className='mt-3 text-slate-300 text-sm sm:text-base leading-relaxed'>
          A vibrant collection of our intensive workshops, hackathon victories, autonomous rover trials, hardware demos, and lab building sprints.
        </p>
      </div>
      ...
      <div className='flex flex-col items-center gap-4 mb-12'>
        ...
        <div className='inline-flex p-1 rounded-2xl bg-slate-950/80 border border-white/10 backdrop-blur shadow-lg'>
          ...
          >
            📷 Photos ({items.filter(i => i.mediaType !== 'video' && !i.videoUrl).length})
          </button>
          <button
          ...
          >
            🎥 Videos ({items.filter(i => i.mediaType === 'video' || !!i.videoUrl).length})
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className='text-center py-16 glass rounded-3xl p-8 border border-white/10 max-w-md mx-auto'>
          <p className='text-lg font-bold text-white font-display'>No media found in this category.</p>
          <p className='text-xs text-slate-400 mt-1'>Check back soon for upcoming lab photos and demo videos!</p>
        </div>
      ) : (
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8'>
          {filteredItems.map(item => {
            const isVideo = item.mediaType === 'video' || !!item.videoUrl;
            const ytId = isVideo && item.videoUrl ? getYouTubeId(item.videoUrl) : null;
            const displayThumbnail = item.image || (ytId ? `https://img.youtube.com/vi/${ytId}/hqdefault.jpg` : '');
                ...
                <div className='absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/30 to-transparent opacity-80 group-hover:opacity-95 transition-opacity flex flex-col justify-between p-6'>
                  <div className='flex justify-between items-start'>
                    <div className='flex flex-wrap items-center gap-1.5'>
                      ...
                      <span className='font-tech text-[9px] text-slate-300 bg-slate-950/70 backdrop-blur px-2 py-0.5 rounded-full border border-white/10'>
                        📅 {item.date ? new Date(item.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : new Date(item.createdAt || Date.now()).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </span>
                    </div>
                    {isVideo && (
                  ...
                  <div>
                    ...
                    <span className='text-[11px] font-tech text-cyan-300 mt-1 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity font-bold'>
                      <span>{isVideo ? 'Click to watch video' : 'Click to expand photo'}</span>
                      <span>↗</span>
                    </span>
                  </div>
                </div>
      ...
      {lightboxItem && (
        ...
        >
          ...
          >
            ...
            {/* Content Display */}
            {(() => {
              const isVideo = lightboxItem.mediaType === 'video' || !!lightboxItem.videoUrl;
              const ytId = isVideo && lightboxItem.videoUrl ? getYouTubeId(lightboxItem.videoUrl) : null;
            ...
            <div className='pt-4 flex items-center justify-between'>
              ...
              <span className='font-tech text-xs text-slate-300 flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10'>
                ...
                <span>
                  {lightboxItem.date
                    ? new Date(lightboxItem.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })
                    : new Date(lightboxItem.createdAt || Date.now()).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                </span>
              </span>
            </div>
          </div>
