    ...
    <div className='max-w-7xl mx-auto px-6 py-14'>
      ...
      <div className='text-center max-w-3xl mx-auto mb-12'>
        ...
        <p className='mt-3 text-slate-300 text-sm sm:text-base leading-relaxed'>
          Explore autonomous rovers, IoT sensor grids, smart automation, and aerial drone systems engineered by student builders.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className='text-center py-16 glass rounded-3xl p-8 border border-white/10 max-w-md mx-auto'>
          <p className='text-lg font-bold text-white font-display'>No hardware projects found.</p>
          <p className='text-xs text-slate-400 mt-1'>Try clearing your search filters or check another technical domain.</p>
          <button
                ...
                <div className='p-6'>
                  ...
                  {p.teamMembers && p.teamMembers.length > 0 && (
                    <div className='mt-4 pt-3 border-t border-white/5 flex items-center gap-2 text-[11px] font-tech text-slate-400'>
                      ...
                      <span className='text-slate-200 truncate'>{p.teamMembers.join(', ')}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fadeIn'>
          <div className='glass-card max-w-2xl w-full rounded-3xl p-6 sm:p-8 border border-cyan-500/40 shadow-2xl shadow-cyan-500/20 relative max-h-[90vh] overflow-y-auto'>
            ...
            <div className='mt-6 space-y-4'>
              <div>
                <h4 className='font-tech text-xs text-cyan-400 uppercase tracking-wider font-bold mb-1'>Architecture Overview</h4>
                <p className='text-sm text-slate-200 leading-relaxed'>{selectedProject.description}</p>
              </div>

              <div>
                <h4 className='font-tech text-xs text-cyan-400 uppercase tracking-wider font-bold mb-2'>Technologies & Frameworks</h4>
                <div className='flex flex-wrap gap-2'>
                  {selectedProject.technologies?.map((tech, i) => (
                    <span key={i} className='text-xs font-tech px-3 py-1 rounded-lg bg-cyan-950/40 border border-cyan-500/25 text-cyan-300 font-medium'>
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              {selectedProject.teamMembers?.length > 0 && (
                <div>
                  <h4 className='font-tech text-xs text-slate-400 uppercase tracking-wider font-bold mb-1'>Lead Builders & Fabricators</h4>
                  <p className='text-xs text-slate-200 font-tech'>{selectedProject.teamMembers.join(' • ')}</p>
                </div>
              )}
